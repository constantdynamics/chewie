...
> Conforms to: the canonical concentric-rings architecture spine (Calm Core / Sensing Layer / Companion Plane), committed at `docs/00-architecture-spine.md`.

### Canonical documentation map (single numbering convention)

All Chewie design docs live under `docs/` with the numbering below. **This is the one canonical scheme; no doc invents its own sibling paths.** A CI link-checker over `docs/` fails the build on any dangling cross-reference (see `docs/00-architecture-spine.md` §CI). Related docs referenced here (not duplicated):

| Path | Owns |
| --- | --- |
| `docs/00-architecture-spine.md` | The canonical spine every doc conforms to; ADR index pointer; CI link-check policy |
| `docs/01-product-vision.md` | **This doc** — vision, personas, core loop, principles, metrics |
| `docs/02-session-engine.md` | `@chewie/engine` XState statechart, `ChewieClock`, keep-awake, background cues, **in-progress session checkpoint & recovery** |
| `docs/03-behavior-scoring.md` | `@chewie/scoring` band math, property tests |
| `docs/04-chewart-generator.md` | `@chewie/art` seeded PRNG, palettes, geometry, deterministic SVG + tolerant raster export |
| `docs/05-sensing-fusion.md` | `@chewie/fusion` scale drivers, camera, `SensorMode`; consumes canonical `BiteEvent` |
| `docs/06-companion-plane.md` | Ring 3 WebRTC/Supabase pairing, RLS, Presence |
| `docs/07-balance-variety-insight.md` | `@chewie/nutrition` optional, off-by-default intake insight; **persisted-session recovery shape** |
| `docs/08-data-model-and-privacy.md` | Encrypted SQLite schema, GDPR/DPIA, age-band field |
| `docs/09-design-system.md` | Tokens (no red/failure states), motion, haptics |
| `docs/10-safeguards-and-onboarding.md` | **First-run/onboarding flow owner**, age gate, disordered-use heuristic, message catalog |
...
3. **Cue through colour and touch, not alarms.** Phase changes are signalled by a soft colour crossfade and an optional gentle haptic — never a jarring sound, never a red flash. There is **no red, no alarm, no failure state** anywhere in the eating experience (`docs/09-design-system.md`).
...
5. **Works beautifully when it fails.** Offline-first, sensor-optional. If BLE drops, the camera is off, or there is no network, you still get the complete calm loop. Degradation is graceful and honest (see §5.2 and `docs/05-sensing-fusion.md`).
...
Els's design is **consent-first and eater-controlled**: view-only, ephemeral, never recorded, revocable, always-visible (`docs/06-companion-plane.md`). Els must *never* become a way to *police* someone's eating (see anti-persona below).

### 4.5 Explicit anti-personas (who we design *against*)

Naming who we refuse to serve is a design tool, not just ethics theatre:

- **The dieter seeking weight loss / calorie restriction.** Chewie has no weight, BMI, calorie budget, or "eat less" gradient. If restriction is your goal, Chewie will feel pointedly indifferent to it — by design.
...
The vision is **concentric rings** (per `docs/00-architecture-spine.md`). Each ring adds capability but *never adds obligation* and *never gates* the ring inside it.
...
- **Four first-class modes** with graceful degradation — `SensorMode = NONE | SCALE_ONLY | CAMERA_ONLY | BOTH` — so every hardware combination is a valid, honest experience (details in `docs/05-sensing-fusion.md`).
...
Full statechart, keep-awake, haptics, background/notification cues **and the recovery mechanism** are specified in `docs/02-session-engine.md`. This section owns the *emotional shape*; that doc owns the *implementation*.
...
**Design commitment (owned by `docs/02-session-engine.md`; persisted shape defined in `docs/07-...` / `docs/08-data-model-and-privacy.md`):**
...
The **end-to-end first-run/onboarding flow is owned by `docs/10-safeguards-and-onboarding.md`** — this vision doc sets the requirements it must satisfy:
...
- **Profile creation is minimal.** No account, no weight, no goal — just age band, language (nl/en), and default timings. Onboarding copy centres digestion, satiety, savouring and calm; **no BMI, no weight, no calorie/diet path exists** (enforced by schema absence, §8 principle 5).
...
Key properties (asserted by property-based tests in `docs/03-behavior-scoring.md`):
...
- Copy is drawn from a constrained catalogue with **no** `failed / broke / lost / missed` templates (`docs/10-safeguards-and-onboarding.md`).
...
2. **Behaviour, never amount.** The primary score measures *how* you eat. "Ate less" can never raise it — enforced by type, not by hope.
3. **Bands, not minimisation.** Healthy ranges with symmetric penalties; no "more/less is always better" slope anywhere.
...
5. **No weight-loss framing, enforced by schema.** No weight/BMI/goal fields exist to build a diet flow on. Onboarding centres digestion, satiety, savouring, calm.
...
- **Not a weight-loss / dieting app.** No weight, BMI, calorie budget, or "eat less" mechanic — not in onboarding, not in the schema.
- **Not a food/macro diary.** No MyFitnessPal-style logging. Intake insight is secondary, optional, non-punitive.
- **Not a scoreboard.** No blended score mixing behaviour with amount; behaviour and intake are separate packages and stay that way.
...
These feed the local, on-device disordered-use safeguard (`docs/10-safeguards-and-onboarding.md`). **Be honest about what the safeguard can and cannot see:**
...
- [ ] No red/failure state, no punitive copy, no weight/BMI/goal field, no intake→score/art code path.
...
**Risks (vision-level; full register in `docs/00-architecture-spine.md` `risks`)**
...
- **Framing drift.** Marketing or a future PM could quietly re-introduce weight-loss framing for growth. Mitigation: the schema has no weight/BMI/goal columns and the banned-terms list (`NutritionScore`, punitive copy) makes drift a code-review-visible act, not a silent one.
